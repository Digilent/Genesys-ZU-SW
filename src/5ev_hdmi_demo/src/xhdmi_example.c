 ...
 */
#define CONTROL_SI5342_LOLN_DEVICE_ID  XPAR_CONTROL_SI5342_LOLN_DEVICE_ID

/*
 * The following constant is used to determine which channel of the GPIO is
 * used for the LOLN pin if there are 2 channels supported.
 */
#define LOLN_CHANNEL 1

#define LOLN_HIGH 0x01   /* Bit 0 of GPIO is connected to the OR gate controlling the LOLN  */
/*
 * The following are declared globally so they are zeroed and so they are
 * easily accessible from a debugger
 */

XGpio LOLN_Gpio; /* The Instance of the GPIO Driver */
...
/**
*
* This function setup SI5342 clock generator over IIC.
*
* @param  None.
*
...
#if defined(XPS_BOARD_GZU_5EV)
	Buffer = 0x04;
	Status = XIic_Send((XPAR_IIC_0_BASEADDR), (I2C_MUX_ADDR), (u8 *)&Buffer, 1, (XIIC_STOP));
#elif defined (ARMR5) || (__aarch64__) && !defined(XPS_BOARD_GZU_5EV)
	Buffer = 0x10;
...
#if (!defined XPS_BOARD_ZCU104) && (!defined XPS_BOARD_GZU_5EV)
	/* Free running mode */
	if (InFreq == 0) {
...
#elif (!defined XPS_BOARD_GZU_5EV)
...
void StartTxAfterRx(void) {
	...
	xil_printf("Disabling TX TMDS clock\r\n");
	XVphy_Clkout1OBufTdsEnable(&Vphy, XVPHY_DIR_TX, (FALSE));

#if (defined XPS_BOARD_GZU_5EV)
	/* Set the LOLN GPIO output to Low */
	XGpio_DiscreteWrite(&LOLN_Gpio, LOLN_CHANNEL, 0);
#endif

	/* Enable RX clock forwarding */
	xil_printf("Enabling RX clock forwarding\r\n");
	...
	while(check_hdmi_lol_status(&Ps_Iic0)){



		if(XVphy_ClkDetGetRefClkFreqHz(&Vphy, XVPHY_DIR_RX) == 0x00){

			/*If the RX REF Clock is equal with 0 there is no need to wait for lock
			 * We break out and wait for TX events*/
		...
		ElapsedTime=1000*(tEnd-tStart)/(COUNTS_PER_SECOND);//ms


		/*Lock Time can vary significantly depending the bandwidth.
		 *For this case, lock time was measured with nominal and fastlock bandwidths set to 100 Hz
		 * The maximum value for this configuration is equal with 300 ms*/

		if(ElapsedTime >= PLL_MAX_LOCK_TIME){

			//Make sure that we are not hanging in this while loop
	...
	xil_printf("Starting the TX Stream\r\n");
	XV_HdmiTxSs_StreamStart(&HdmiTxSs);
#if (defined XPS_BOARD_GZU_5EV)
/*Reset the HDMI Clock detector after we started the TX Stream*/
	XVphy_ClkDetFreqReset(&Vphy, 0,	XVPHY_DIR_TX);
#endif
	if (!XVphy_IsBonded(&Vphy, 0, XVPHY_CHANNEL_ID_CH1)) {
		xil_printf("GT TX and RX are not coupled/bonded!\r\n");
	} else {
#if (!defined XPS_BOARD_ZCU104) && (!defined XPS_BOARD_GZU_5EV)
	...
	/* When the GT TX and RX are coupled, then start the TXPLL */
	if (XVphy_IsBonded(&Vphy, 0, XVPHY_CHANNEL_ID_CH1)) {
		xil_printf("GT TX and RX are coupled/bonded!\r\n");
		/* Config and Run the TPG */
		xil_printf("Configuring and running the TPG.\r\n");
		XV_ConfigTpg(&Tpg);
		/* Start TX MMCM */
		xil_printf("Starting TX MMCM.\r\n");
		XVphy_MmcmStart(&Vphy, 0, XVPHY_DIR_TX);
		usleep(10000);
	}
	...
	if (XVphy_IsBonded(&Vphy, 0, XVPHY_CHANNEL_ID_CH1)) {
		xil_printf("Enabling TX TMDS clock in bonded mode.\r\n");
...
void EnableColorBar(XVphy                *VphyPtr,
	...
	if (!CheckTxBusy()) {
		...
		/* Set the LOLN GPIO output to High */
		XGpio_DiscreteWrite(&LOLN_Gpio, LOLN_CHANNEL, LOLN_HIGH);
...
int main() {
	...
	Status = XGpio_Initialize(&LOLN_Gpio, CONTROL_SI5342_LOLN_DEVICE_ID);
	if (Status != XST_SUCCESS) {
		xil_printf("LOLN Gpio Initialization Failed\r\n");
		return XST_FAILURE;
	}
#endif
	...
	// Set the I2C MUX in order to access SI5342 and the HDMI TX and RX buffers.
#if (defined XPS_BOARD_GZU_5EV)
	/* On Board SI5432 chip for DRU reference clock */
	I2cMux();

	/*Set SI5342 I2C drive level to 3.3V.*/
	Status = Si5342_SetI2cDriveLevel();
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
#else
	...
	/* Initialize external clock generator */
#if (defined XPS_BOARD_GZU_5EV)
	hdmi_config_interface_get_SI5432_write_config_registers((uint8_t)I2C_SI5342_ADDR, si5342_revd_registers_jitteratt, SI5342_REVD_REG_CONFIG_NUM_REGS);
	/* Initialize external clock generator */
#elif (!defined XPS_BOARD_GZU_5EV) || (!defined XPS_BOARD_ZCU104)
	Si5324_Init(XPAR_IIC_0_BASEADDR, I2C_CLK_ADDR);
#else
	usleep(200000);
	IDT_8T49N24x_Init(XPAR_IIC_0_BASEADDR, I2C_CLK_ADDR);
...
/* HDCP 2.2 */
	...
	/* Get User Edid Info */
	XV_HdmiRxSs_SetEdidParam(&HdmiRxSs, (u8*)&Edid3, sizeof(Edid3));
	...
	do {
		...
		else if (TxRestartColorbar && TxSinkReady) {
#if (XPAR_VPHY_0_TRANSCEIVER == XVPHY_GTXE2)
			if (!PLLBondedCheck()) {
#endif
		...
		if (IsTxStreamUp && TxSinkReady) {
			IsTxStreamUp = FALSE;

			i2c_dp159(&Vphy, 0, TxLineRate);
#if (defined XPS_BOARD_GZU_5EV)
			i2c_tmds181(&Vphy, 0);
			i2c_buffer_clear_ber(I2C_DP159_ES_ADDR);
			i2c_buffer_clear_ber(TMDS181_ADDR);
			i2c_buffer_enable_verify_datapath(I2C_DP159_ES_ADDR);
			i2c_buffer_enable_verify_datapath(TMDS181_ADDR);
#endif
